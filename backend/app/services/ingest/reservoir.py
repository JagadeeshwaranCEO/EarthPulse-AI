...
class ReservoirAdapter(DataSourceAdapter):
    id = "reservoir"
    kind = "water"
    description = ("Tamil Nadu PWD/WRD reservoir storage, level and release. "
            ...
            for loc in locations:
                ...
                for row in r.json().get("dams", []):
                    frames.append(_frame(
                        loc["id"], "reservoir_storage_pct", row["storage_pct"],
                        API_SOURCE, unit="%", at=datetime.fromisoformat(row["ts"])))
                    frames.append(_frame(
                        loc["id"], "reservoir_release_m3s", row.get("release_m3s", 0),
                        API_SOURCE, unit="m3/s", at=datetime.fromisoformat(row["ts"])))
        return frames
    ...
    def _fetch_demo(self, locations, since=None):
        ...
        for loc in locations:
            frames.append(_frame(loc["id"], "reservoir_storage_pct",
                                 _demo_derived(loc, "storage_pct", 82.0),
                                 DEMO_SOURCE, unit="%", at=now))
            frames.append(_frame(loc["id"], "reservoir_release_m3s",
                                 _demo_derived(loc, "release_m3s", 45.0),
                                 DEMO_SOURCE, unit="m3/s", at=now))
        return frames
