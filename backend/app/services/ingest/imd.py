...
class IMDAdapter(DataSourceAdapter):
    id = "imd"
    kind = "weather"
    description = ("IMD rainfall — hourly accumulation per monitored zone. "
                   "Live when IMD_ENDPOINT is set; else labeled demo.")
            ...
            for loc in locations:
                ...
                for row in r.json().get("readings", []):
                    frames.append(_frame(
                        loc["id"], "rainfall_mm", row["rain_mm"], API_SOURCE,
                        unit="mm", at=datetime.fromisoformat(row["ts"])))
        return frames
    ...
    def _fetch_demo(self, locations, since=None):
        ...
        for loc in locations:
            frames.append(_frame(loc["id"], "rainfall_mm",
                                 _demo_derived(loc, "rainfall_mm", 4.0),
                                 DEMO_SOURCE, unit="mm", at=now))
        return frames
