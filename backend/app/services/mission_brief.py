...
def build_mission_brief(*, strategy: StrategyOption, top_risks: list[dict],
                        peak_risk: float, peak_at_hour: int, now_hour: int,
    ...
    for r in top_risks[:5]:
        lines.append(f"- **{r['location_name']}** — risk {r['risk_probability']:.0%} "
    ...
    if trust:
        lines.append(f"- **Data trust: {trust.get('level', '—')}** ({trust.get('score', 0):.0f}/100) — "
        ...
        lines.append(f"- Prediction confidence: {analysis.get('prediction_confidence', 0):.0%} · "
    ...
    if decision_conf is not None:
        lines.append(f"- **Decision confidence: {decision_conf:.0%}** — probability the selected plan "
                     f"remains optimal under precipitation variance.")
        if fallback_id:
            lines.append(f"- If forecast degrades: **switch to {fallback_id}** ({fallback_trigger}).")
        else:
            lines.append(f"- No strategy switch required across the {{0.85–1.60}}× variance sweep.")
    else:
    ...
    for sid, meta in trade_offs.items():
        ...
        lines.append(f"- **{sid.replace('strat_', 'Strategy ').title()}** {sel} — "
    ...
    for a in strategy.actions:
        lines.append(f"- {a}")
    lines.append("")
    lines.append("_EarthPulse AI · simulation-informed, not predicted-by-LLM. Confidence bounds "
    ...
    return {
        ...
        "facets": {
            "situation": [
                {"name": r["location_name"], "p": r["risk_probability"], "level": r["level"]}
                for r in top_risks[:5]
            ],
            "decision": {
