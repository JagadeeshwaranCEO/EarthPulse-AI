...
def _tsu_causal(components: dict[str, float], pred: dict) -> dict:
    nodes = [
        {
            "id": "src",
            "label": "Offshore seismic energy",
            "kind": "cause",
            "value": f"{components.get('source_energy', 0) * 30:.0f} GJ",
            "confidence": 0.85,
        },
        {
            "id": "sea",
            "label": "Sea-surface disturbance",
            "kind": "mechanism",
            "value": f"{components.get('sea_disturbance', 0):.2f}/12",
            "confidence": 0.8,
        },
        {
            "id": "coast",
            "label": "Coastal surge exposure",
            "kind": "condition",
            "value": f"{components.get('coastal_exposure', 0):.2f}/12",
            "confidence": 0.75,
        },
        {
            "id": "report",
            "label": "Sea-state reports",
            "kind": "condition",
            "value": f"{components.get('sea_state_report', 0):.1f} reports",
            "confidence": 0.6,
        },
        {
            "id": "risk",
            "label": f"Tsunami risk ({pred.get('risk_probability', 0):.0%})",
            "kind": "risk",
...
TSUNAMI = HazardSpec(
    ...
    interventions=[
        {
            "id": "shoreline_evacuation",
            "name": "Shoreline Evacuation",
            "kind": "policy",
            "description": "Evacuate the surge corridor to high-ground contour",
        },
        {
            "id": "high_ground_staging",
            "name": "High-Ground Staging",
            "kind": "operational",
            "description": "Move rescue assets behind the surge line",
        },
        {
            "id": "coastal_substation_isolation",
            "name": "Coastal Grid Isolation",
            "kind": "engineering",
            "description": "Isolate coastal substations ahead of water arrival",
        },
        {
            "id": "tide_gauge_monitoring",
            "name": "Tide Gauge Monitoring",
            "kind": "operational",
    ...
    evidence=[
        ...
        EvidenceTemplate(
            "gpm-nasa", "observation", "surface-water disturbance ahead of the shore", "sea_disturbance", 2
        ),
        EvidenceTemplate(
            "civic-reports", "report", "reported sea recession or standing water along the shore", "sea_state_report", 1
        ),
        EvidenceTemplate("news-eom", "citation", "tsunami advisory active for the coastal district", None, 6),
    ],
    hourly=_tsu_hourly,
    causal=_tsu_causal,
