    ...
    nodes = [
        {
            ...
            "kind": "cause",
            "value": f"{components.get('precipitation_deficit', 0):.2f}/12",
            "confidence": 0.9,
        },
        {
            "id": "soil",
            "label": "Soil desiccation",
            "kind": "mechanism",
            "value": f"{components.get('soil_desiccation', 0):.2f}/12",
            "confidence": 0.85,
        },
        {
            "id": "heat",
            "label": "Heat stress",
            "kind": "mechanism",
            "value": f"{components.get('heat_stress', 0):.2f}/12",
            "confidence": 0.8,
        },
        {
            "id": "scarcity",
            "label": "Water scarcity reports",
            "kind": "condition",
            "value": f"{components.get('water_scarcity', 0):.1f} reports",
            "confidence": 0.6,
        },
        {
            "id": "risk",
            "label": f"Drought risk ({pred.get('risk_probability', 0):.0%})",
            "kind": "risk",
...
DROUGHT = HazardSpec(
    ...
    interventions=[
        {
            "id": "tanker_corridors",
            "name": "Water Tanker Corridors",
            "kind": "operational",
            "description": "Tanker supply for scarcity-report clusters",
        },
        {
            "id": "volumetric_rationing",
            "name": "Volumetric Rationing",
            "kind": "policy",
            "description": "Rationing ahead of reservoir trigger lines",
        },
        {
            "id": "borefield_protection",
            "name": "Borefield Protection",
            "kind": "engineering",
        ...
        {
            "id": "reservoir_release_plan",
            "name": "Reservoir Release Plan",
            "kind": "operational",
    ...
    evidence=[
        ...
        EvidenceTemplate(
            "gpm-nasa", "observation", "soil moisture anomaly in persistent deficit", "soil_desiccation", 4
        ),
        EvidenceTemplate("civic-reports", "report", "reported dry taps and failing borewells", "water_scarcity", 2),
        EvidenceTemplate("news-eom", "citation", "drought advisory active for the district", None, 8),
    ],
    hourly=_dr_hourly,
    causal=_dr_causal,
