...
def _ls_causal(components: dict[str, float], pred: dict) -> dict:
    nodes = [
        {"id": "sat", "label": "Slope saturation", "kind": "cause",
         "value": f"{components.get('slope_saturation', 0):.2f}/12", "confidence": 0.85},
        {"id": "rain", "label": "Rain burst trigger", "kind": "cause",
         "value": f"{components.get('rain_trigger', 0):.2f}/12", "confidence": 0.85},
        {"id": "terrain", "label": "Terrain fragility", "kind": "condition",
         "value": f"{components.get('terrain_fragility', 0):.2f}/12", "confidence": 0.75},
        {"id": "slip", "label": "Slippage reports", "kind": "condition",
         "value": f"{components.get('slippage_report', 0):.1f} reports", "confidence": 0.6},
        {"id": "risk", "label": f"Landslide risk ({pred.get('risk_probability', 0):.0%})", "kind": "risk",
...
def _ls_recommend(p: float, sev: float) -> list:
    return [
        {"id": "rec_ls_1", "stakeholder": "responders", "priority": 1 if p > 0.6 else 2,
         ...
         "evidence_ids": []},
        {"id": "rec_ls_2", "stakeholder": "civic", "priority": 1 if sev >= 3 else 2,
         ...
         "evidence_ids": []},
        {"id": "rec_ls_3", "stakeholder": "utilities", "priority": 2,
         ...
         "evidence_ids": []},
        {"id": "rec_ls_4", "stakeholder": "public", "priority": 3,
...
LANDSLIDE = HazardSpec(
    ...
    interventions=[
        {"id": "corridor_closure", "name": "Hillside Corridor Closure", "kind": "policy",
         "description": "Close runout corridors during active saturation"},
        {"id": "slope_relocation", "name": "Slope-Base Relocation", "kind": "policy",
         "description": "Relocate settlements at monitored slope bases"},
        {"id": "debris_drain_clearing", "name": "Debris Drain Clearing", "kind": "engineering",
         "description": "Protect culverts and cuts from debris blockage"},
        {"id": "slope_anchoring", "name": "Slope Anchoring", "kind": "engineering",
         "description": "Install anchors and drainage on critical faces"},
        {"id": "monitoring_teams", "name": "Slope Monitoring", "kind": "operational",
         "description": "Crack-meter and gauge watch on active faces"},
    ],
    evidence=[
        EvidenceTemplate("imd-rain", "observation",
                         "rain burst above the slope-failure intensity threshold",
                         "rain_trigger", 2),
        EvidenceTemplate("gpm-nasa", "observation",
                         "soil saturation above the mobilization floor",
                         "slope_saturation", 2),
        EvidenceTemplate("civic-reports", "report",
                         "reported cracks, tilting poles and springs",
                         "slippage_report", 1),
        EvidenceTemplate("news-eom", "citation",
                         "landslide watch active for the hill district",
                         None, 6),
    ],
    hourly=_ls_hourly,
    causal=_ls_causal,
