...
def get_roster(db: Session = Depends(get_db)):
    ...
    for name, agent in ROSTER.items():
        last = db.query(AgentMessage).filter_by(agent=name).order_by(desc(AgentMessage.created_at)).first()
        roster.append(
            {
                "name": name,
...
def get_pipeline(db: Session = Depends(get_db)):
    return [
        db.query(AgentMessage).filter_by(agent=name).order_by(desc(AgentMessage.created_at)).first().content
        if db.query(AgentMessage).filter_by(agent=name).count()
        else "pending"
...
async def get_debate(
    topic: str = "Chennai monsoon flood risk", risk_id: str = "", force: bool = False, db: Session = Depends(get_db)
):
    pred = None
    if risk_id:
        pred = (
